...
# as "12" which means frequency = 1 and value = 2, the same way "1" is read as "11", so the answer 
# is the concatenation of "12" and "11" which is "1211".

class Solution:
    def countAndSay(self, n: int, output = "1") -> str:
        if(n==1):
            return(output)
        else:
            temp = ""
            ...
            for i in output:
                if i != temp:
                    if count>0:
                        newOutput += str(count) + temp            
                    temp = i
                    count = 1
                else:
                    count+=1
            newOutput += str(count) + temp
            return(self.countAndSay(n=n-1, output=newOutput))
